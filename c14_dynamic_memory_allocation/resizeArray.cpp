...
int main()
{
  ...
  while (val > 0)
  {
    ...
    {
      // Now we need to call growArray()
      // We need to pass size as a pointer since we need
      // to keep track of the size of the array as it grows
...
// a good idea is to double the size of the array to limit
// the number of times we have to copy everything
int* growArray(int* p_values, int* size)
{
  *size *= 2;
